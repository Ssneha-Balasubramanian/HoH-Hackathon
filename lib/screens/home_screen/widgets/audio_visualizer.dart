...
class AudioVisualizer extends StatelessWidget {
  AudioVisualizer({required this.amplitude}) {
    ...
    double db = amplitude ?? RecorderConstants.decibelLimit;
    if (db == double.infinity || db < RecorderConstants.decibelLimit) {
      db = RecorderConstants.decibelLimit;
    }
    if (db > 0) {
      db = 0;
    }
    ...
    range = 1 - (db * (1 / RecorderConstants.decibelLimit));
    print(range);
  }
